...
import {
  closestObservationField,
  closestStationWithObservationField,
  getDistance,
} from "../utils/distance";
...
export default function Main(props) {
  const [allObservations, setAllObservations] = useState<Observations>(
    undefined
  );
  ...
  useEffect(() => {
    if (allObservations && location) {
      ...
      let closest: Station = closestStationWithObservationField(
      ...
      setObservations({
        ...observations,
        station: stationsWithDistance.sort((a, b) =>
  ...
  const getWaterTempStation = () =>
    closestStationWithObservationField(
      observations.station,
      "watertemperature"
    );
  const getPhenomenonStation = () =>
    closestStationWithObservationField(observations.station, "phenomenon");
  const getWindSpeedStation = () =>
    closestStationWithObservationField(observations.station, "windspeed");
  const getWindSpeedMax = () =>
    closestStationWithObservationField(observations.station, "windspeedmax");
  const getHumidity = () =>
    closestStationWithObservationField(
      observations.station,
      "relativehumidity"
    );
  const getPrecipitations = () =>
    closestStationWithObservationField(observations.station, "precipitations");
  const getUVIndex = () =>
    closestStationWithObservationField(observations.station, "uvindex");

  const phenomenon = observations
    ? getPhenomenonText(getPhenomenonStation().phenomenon)
    : "";
  return (
    <View style={{ flex: 1 }}>
      <Text
            ...
            >
              <View style={styles.container}>
                ...
                <View style={styles.smallContainer}>
                  <Text style={styles.smallText}>
                    {closestStation.name},{" "}
                    {addZeroBefore(latestUpdate.getHours())}:
                    {addZeroBefore(latestUpdate.getMinutes())}
                  </Text>
                  <Text style={styles.smallText}>
                    vesi {getWaterTempStation().watertemperature}°C{" "}
                    {showDataOrigin && (
                      <Text style={styles.smallText}>
                        ({getWaterTempStation().name})
                      </Text>
                    )}
                  </Text>
                  <Text style={styles.smallText}>
                  ...
                  <Text style={styles.smallerText}>
                    õhuniiskus {getHumidity().relativehumidity}%{" "}
                    {showDataOrigin && (
                      <Text style={styles.smallerText}>
                        ({getHumidity().name})
                      </Text>
                  ...
                  <Text style={styles.smallerText}>
                    sademeid tunnis {getPrecipitations().precipitations}mm{" "}
                    {showDataOrigin && (
                      <Text style={styles.smallerText}>
                        ({getPrecipitations().name})
                      </Text>
                    )}
                  </Text>
                  <Text style={styles.smallerText}>
